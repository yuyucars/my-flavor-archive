...
【材料抽出の最重要ルール】
テキスト中に「★合わせ調味料」「☆肉下味」のような記号付きのグループ見出しが現れた場合、
そのグループに属する材料すべての名前の先頭に、そのグループの記号を付けてください。

例：テキストに「★合わせ調味料 しょうゆ 大さじ2 みりん 大さじ1」とある場合
→ { "name": "★しょうゆ", "amount": "大さじ2" }
→ { "name": "★みりん", "amount": "大さじ1" }

JSON-LDやschema.orgの構造化データではなく、ページの本文テキストを優先して読んでください。
記号（★☆◎●など）はレシピの作り方で参照されるので、絶対に削除しないでください。`,
        },
      ],
      temperature: 0.1,
    })
